...
For example [`TypeBounds`](https://scala-lang.org/api/3.x/api/scala/quoted/Quotes$reflectModule.html#TypeBounds-0), a subtype of `TypeRepr`, represents a type tree of the form `T >: L <: U`: a type `T` which is a super type of `L`
and a subtype of `U`. In [`TypeBoundsMethods`](https://scala-lang.org/api/3.x/api/scala/quoted/Quotes$reflectModule$TypeBoundsMethods.html), you will find the methods `low` and `hi`, which allow you to access the
representations of `L` and `U`. In [`TypeBoundsModule`](https://scala-lang.org/api/3.x/api/scala/quoted/Quotes$reflectModule$TypeBoundsModule.html), you will find the `unapply` method, which allows you to write:
...
In addition `Symbol` exposes and is used by many useful methods. For example:
 ...
 - and many useful others that you can find in [`SymbolMethods`](https://scala-lang.org/api/3.x/api/scala/quoted/Quotes$reflectModule$SymbolMethods.html)
...
[reflection doc]: https://scala-lang.org/api/3.x/api/scala/quoted/Quotes$reflectModule.html
