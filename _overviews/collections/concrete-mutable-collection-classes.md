...
You saw immutable stacks earlier. There is also a mutable version, supported by class [mutable.Stack](https://www.scala-lang.org/api/{{ site.scala-212-version }}/scala/collection/mutable/Stack.html). It works exactly the same as the immutable version except that modifications happen in place.
...
A concurrent map can be accessed by several threads at once. In addition to the usual [Map](https://www.scala-lang.org/api/{{ site.scala-212-version }}/scala/collection/Map.html) operations, it provides the following atomic operations:
