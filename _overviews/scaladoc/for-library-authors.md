...
Linking to a class:
    ...
    /** Parses input using {@link scala.util.Try} for safe error handling. */
    def parse(s: String): Try[Int] = Try(s.toInt)

Linking to a method:

    /** See {@link scala.collection.Seq#map} for transforming elements. */
    def transform[A, B](xs: Seq[A], f: A => B): Seq[B] = xs.map(f)

Linking to a companion object:

    /** Factory methods are available in {@link scala.concurrent.Future$}. */
    def async[A](body: => A): Future[A] = Future(body)

Linking with description text:

    /** Creates a new date using {@link java.time.LocalDate#now current system date}. */
