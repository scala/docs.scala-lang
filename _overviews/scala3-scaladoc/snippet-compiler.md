...
### Inline diagnostic expectations

The `sc:fail` flag checks that a snippet doesn't compile, but it doesn't care _where_ the error occurs. Sometimes you want to be more precise and assert that specific lines produce errors or warnings. For this, we added inline diagnostic expectations.

To use this feature, add the `+test` modifier to a `compile` or `fail` flag. For example, in sbt:

```scala
Compile / doc / scalacOptions ++= Seq("-snippet-compiler:docs=compile+test")
```

Then annotate expected errors and warnings directly in your snippets with comments:

- `// error` - expect a compilation error on this line
- `// warn` - expect a compilation warning on this line

You can place multiple markers on the same line when you expect multiple diagnostics:
...
With `+test` enabled, the snippet compiler checks that annotations and actual diagnostics match up strictly:

- Snippets without any annotations must compile cleanly, with no errors or warnings.
- Every error or warning the compiler produces must have a matching `// error` or `// warn` on the correct line.
- Every annotation must correspond to an actual diagnostic. If an expectation goes unfulfilled, validation fails.
- Snippets marked `sc:fail` with `+test` need at least one `// error` annotation or at least one actual compilation error.
...
On top of flags, you can append modifiers with `+` to further control snippet behavior:
- `test` - enables inline diagnostic expectations (`// error`, `// warn`). See [Inline diagnostic expectations](#inline-diagnostic-expectations) for details.

For example, `compile+test` turns on snippet checking and verifies that diagnostics match your annotations.
...
Modifiers can be combined with path-based settings:
```
-snippet-compiler:docs=compile+test,library/src=compile
```
