...
Обратите внимание, что объявление типа `Int` происходит после идентификатора `x`. Необходимо также использовать `:`.  
...
```
Возвращаемый тип метода `greet` это `Unit`, используется тогда, когда не имеет смысла что-либо возвращать. Аналогично `void` в Java и C. (Поскольку каждое выражение Scala должно иметь какое-то значение, поэтому используется одиночка (сингэлтон) типа Unit, который задается как (), он не несет какой-либо информации.)
...
Позже мы рассмотрим классы [подробнее](class.html).
...
Трейты - это типы, содержащие определенные поля и методы. Трейты можно комбинировать.

Задать трейт можно с помощью ключевого слова `trait`.
...
Трейты также могут иметь реализации по умолчанию.
