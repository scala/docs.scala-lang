...
|`implicit def attachMethod(i:Int) = new {def tenTimes = i*10}`|another implicit conversion from "Int" to an anonymous class having a method "tenTimes". the returned class doesn't need a name.|
...
|`type Complex[A] = (String,String,String,A)`|if you are too lazy to write a complex type expression because a part of the expression is constant, use this. "Complex" is called a type contructor, and A is its parameter. you can declare something as Complex[Int] and its type is equal to that of a (String, String, String, Int)-tuple|
|`type StructualType = {def x:String;val y:Int}`|this is a structural type. it allows type safe duck-typing. if you declare a method that accepts "StructuralType", it accepts all objects that have a method x:String and a field y:Int. at runtime, reflection is used for this, but you cannot mess it up at compile time like in dynamically typed languages. when creating an anonymous class having additional fields and methods, you can still access them via structural types. the compiler also infers their structural type for you if you write `val foo = new {...}`. you can go ahead and just call your new methods on foo directly, it will compile.| 
