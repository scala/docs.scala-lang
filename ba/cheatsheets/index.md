...
|  `type R = Double`                                                                                       |  alijas za tip     |
|  `def f(x: R)` vs.<br> `def f(x: => R)`                                                                  |  poziv-po-vrijednosti <br> poziv-po-imenu (lijeni parameteri) |
|  `(x:R) => x*x`                                                                                          |  anonimna funkcija  |
|  `(1 to 5).map(_*2)` vs.<br> `(1 to 5).reduceLeft( _+_ )`                                                |  anonimna funkcija: donja crta odgovara argumentu po poziciji. |
...
|  <span class="label important">Loše</span>`var x,y,z = (1,2,3)`                                           |  skrivena greška: svim varijablama dodijeljena cijela n-torka. |
...
|  `for (x <- xs; y <- ys) {`<br>    `println("%d/%d = %.1f".format(x,y, x*y))`<br>`}`                     |  for comprehension: imperativ-asto<br>[sprintf-stil](http://java.sun.com/javase/6/docs/api/java/util/Formatter.html#syntax) |
|  `for (i <- 1 to 5) {`<br>    `println(i)`<br>`}`                                                        |  for comprehension: iteracija uključujući gornju granicu |
|  `for (i <- 1 until 5) {`<br>    `println(i)`<br>`}`                                                     |  for comprehension: iteracija ne uključujući gornju granicu |
|  <span id="pattern_matching" class="h2">podudaranje uzorka(pattern matching)</span>                                                         |                 |
...
|  `class C(var x: R) {`<br>`assert(x > 0, "positive please")`<br>`var y = x`<br>`val readonly = 5`<br>`private var secret = 1`<br>`def this = this(42)`<br>`}`|<br>konstruktor je tijelo klase<br>deklaracija javnog člana<br>deklaracija dostupnog ali ne promjenjivog člana<br>deklaracija privatnog člana<br>alternativni konstruktor|
|  `new{ ... }`                                                                                            |  anonimna klasa |
...
|  `trait T { ... }`<br>`class C extends T { ... }`<br>`class C extends D with T { ... }`                  |  traitovi.<br>interfejs-s-implementacijom. bez parametara konstruktora. [miksabilan]({{ site.baseurl }}/tutorials/tour/mixin-class-composition.html).
|  `trait T1; trait T2`<br>`class C extends T1 with T2`<br>`class C extends D with T1 with T2`             |  više traitova. |
