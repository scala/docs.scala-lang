...
- **影射** 包含键/值对的集合，例如 Java `Map`、Python 字典或 Ruby `Hash`
- **集合** 是无重复元素的无序集合
...
在接下来的部分中，无论何时使用_不可变_这个词，都可以安全地假设该类型旨在用于_函数性编程_(FP) 风格。
使用这些类型，您无需修改​​集合；您将功能方法应用于该集合以创建新的结果。
